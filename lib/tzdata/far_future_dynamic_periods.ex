defmodule Tzdata.FarFutureDynamicPeriods do
  ...
  @greg_sec_30_years_from_now :calendar.datetime_to_gregorian_seconds(
                                {{(:calendar.universal_time() |> elem(0) |> elem(0)) + 30, 2, 1},
                                 {0, 0, 0}}
                              )
  ...
  def periods_for_point_in_time({{year, _month, _day}, _}, zone_name) do
    ...
    {:ok,
     periods_until_year(
       [first_period],
       first_period.until.utc,
       first_period.utc_off,
       fp_data.zone_line,
       rules,
       year,
       rules_per_year
  ...
  def periods_for_point_in_time(gregorian_seconds, zone_name) do
    periods_for_point_in_time(
      gregorian_seconds |> :calendar.gregorian_seconds_to_datetime(),
      zone_name
    )
  end
  ...
  defp periods_until_year(prev_periods, from, utc_off, zone_line, rules, year, rules_per_year) do
    ...
    until_time_year =
      case length(prev_periods) >= rules_per_year do
        true -> year + 1
        false -> year
      end
    ...
    until_utc =
      Util.datetime_to_utc(Util.time_for_rule(end_rule, until_time_year), utc_off, std_off)
  ...
  defp first_period_that_ends_in_year(zone_name, year) do
    ...
    from =
      Util.datetime_to_utc(
        Util.time_for_rule(rule_end_of_year, year - 1),
        utc_off,
        std_off_before
      )
    ...
    until_utc =
      Util.datetime_to_utc(Util.time_for_rule(rule_beginning_of_year, year), utc_off, std_off)
